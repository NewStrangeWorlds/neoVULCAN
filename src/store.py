...
class Variables(object):
    ...
    def __init__(self): # self means the created object instance
        ...
        self.dt = cfg.solver.dttry # time step
        ...
        sflux_data = np.genfromtxt(cfg.paths.sflux_file, dtype=float, skip_header=1, names = ['lambda', 'flux'])
        ...
        if cfg.photochemistry.use_photo: 
            ...
            if cfg.photochemistry.T_cross_sp: self.var_save.extend(['cross_J','cross_T'])
            if cfg.photochemistry.use_ion: self.var_save.extend(['charge_list', 'ion_sp', 'cross_Jion','Jion_sp', 'ion_wavelen','ion_branch','ion_br_ratio'])
...
class AtmData(object):
    ...
    """
    def __init__(self):
        self.pco = np.logspace(np.log10(cfg.atmosphere.P_b),np.log10(cfg.atmosphere.P_t),nz) # pressure grids
        ...
        self.gas_indx = [_ for _ in range(ni) if spec_list[_] not in cfg.condensation.non_gas_sp]
                    
        self.fix_sp_indx = {}
        if cfg.condensation.fix_species:
        ...
        if cfg.photochemistry.use_ion: self.fix_e_indx = np.arange(spec_list.index('e'), spec_list.index('e') + ni*nz, ni)
        
        self.r_p, self.rho_p = {}, {}
        if cfg.condensation.use_condense:
            for sp in cfg.condensation.r_p.keys():
                self.r_p[sp] = cfg.condensation.r_p[sp]
            for sp in cfg.condensation.rho_p.keys():
                self.rho_p[sp] = cfg.condensation.rho_p[sp]  
